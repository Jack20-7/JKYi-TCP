...
j_addr_pool* CreateAddressPool(in_addr_t addr_base,int num_addr){
    ...
    ap = (j_addr_pool*)calloc(1,sizeof(j_addr_pool));
    if(!ap){
        return NULL;
    }

    num_entry = num_addr * (J_MAX_PORT - J_MIN_PORT);
    ap->pool = (j_addr_entry*)calloc(num_entry,sizeof(j_addr_entry));
    if(!ap->pool){
        free(ap);
        return NULL;
    }

    ap->mapper = (j_addr_map*)calloc(num_addr,sizeof(j_addr_map));
    if(!ap->mapper){
        free(ap->pool);
        free(ap);
        return NULL;
    ...
    for(i = 0;i < num_addr;++i){
        ...
        addr = htonl(addr_h);
        for(j = J_MIN_PORT;j < J_MAX_PORT;++j){
...
j_addr_pool* CreateAddressPoolPerCore(int core,int num_queues,
    ...
    num_entry = (num_addr * (J_MAX_PORT - J_MIN_PORT)) / num_queues;
    ap->pool = (j_addr_entry *)calloc(num_entry, sizeof(j_addr_entry));
    if (!ap->pool) {
        free(ap);
        return NULL;
...
///根据地址的ip + port + 截至ip + 截至port计算出一个hash值
static uint32_t 
